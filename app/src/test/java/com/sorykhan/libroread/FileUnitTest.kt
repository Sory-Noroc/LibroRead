...
class FileUnitTest {
    ...
    fun get_pdf_info_test() {
        ...
        val file = File(path)
        assertEquals(PdfUtils.getPdfInfo(file), Triple("ConfirmareCraiova.pdf", path, 339_184L))
    }

    @Test
    fun get_PDFs_from_Documents_test() {
        val pdfDocsList = PdfUtils.getPDFs("C:\\Users\\Admin\\OneDrive\\Documente")
    ...
    fun get_PDFs_from_nothing_test() {
        val emptyList = PdfUtils.getPDFs("")
        assertEquals(emptyList, listOf<File>())
    }

    @Test
    fun get_all_PDFs_from_desktop_test() {
        val pdfList = PdfUtils.getPDFsRecursively("C:\\Users\\Admin\\OneDrive\\Desktop").map { it.name }
        assertEquals(
            pdfList,
    ...
    fun get_pdf_info_from_documents_test() {
        val pdfList = PdfUtils.getAllPdfInfo(PdfUtils.getPDFs("C:\\Users\\Admin\\OneDrive\\Documente"))
        assertEquals(
            pdfList,
            listOf(Triple(
                "AdeverintaMedicala.pdf",
